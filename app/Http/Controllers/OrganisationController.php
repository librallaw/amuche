...
class OrganisationController extends Controller
{
    ...
    public function doCreateOrganisation(Request $request, Messenger $messenger)
    {
        $request -> validate([
            'name' => 'required',
      ...
      //  dd($unique_id);
        ...
        $messenger->log(Auth::user()->unique_id,"Create Organisation ", $user->unique_id,"Successfully created Organisation",1);
    ...
    public function showOrganisationProfile(Messenger $messenger,$orgid)
    {
        ...
        $data['logs'] = Accountlog::where("unique_id",$orgid)->where("action","Verify")->paginate(10);
        $data['total'] = Accountlog::where("unique_id",$orgid)->where("action","Verify")->count();
        $data['success'] = Accountlog::where("unique_id",$orgid)->where("action","Verify")->where("status",1)->count();
        $data['failed'] = Accountlog::where("unique_id",$orgid)->where("action","Verify")->where("status",0)->count();
      ...
      //  dd( $data['account']);
        ...
        if(Auth::user()->level == 2) {
            ...
            return view("organisation.profile2", $data);
        }
    ...
    public function doCreditWallet(Request $request, Messenger $messenger)
    {
        $request -> validate([
        ...
        if(Auth::user()->level == 1) {
            ...
            if (Hash::check($request->password, Auth::user()->password)) {
                ...
                if (!empty($wallet)) {
                    ...
                    $messenger->log(Auth::user()->unique_id, "Credit Wallet", $request->orgid, "Added: {$request->amount}, Previous: {$prev}, New: {$wallet-> balance} ", 1);
        ...
        if(Auth::user()->level == 2) {
           ...
           // dd($sub_user);
            ...
            if(!empty($wallet)){
                ...
                $messenger->log($request->orgid,"Credit Wallet", $request->orgid,"Added: {$request->amount}, Previous: {$prev}, New: {$wallet-> balance} ",1);


                $messenger->log(Auth::user()->unique_id,"Disburse Credit", $request->orgid,"Disbursed: {$request->amount}, Previous: {$prev_parent}, New: {$balance-> balance}  Organisation: {$request->orgid} ",1);
                ...
                $messenger->log(Auth::user()->unique_id,"Disburse Credit".$request->orgid, $request->orgid,"Disbursed: {$request->amount}, Previous: {$prev_parent}, New: {$balance-> balance}  Organisation: {$request->orgid} ",1);
